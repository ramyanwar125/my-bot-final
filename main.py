...
# --- 1. الإعدادات ---
TOKEN = "8579186374:AAEUzOGQ8y6jIjYWRkOKM_x7QhB1xaiyZSA" # استبدله بالتوكن الخاص بك
ADMIN_ID = 7349033289 
DEV_USER = "@TOP_1UP"   
...
async def check_access(update, context):
    ...
    for ch in CHANNELS:
        try:
            ...
            if member.status in ['left', 'kicked', 'member']: # تم السماح بـ member
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "المطور" in text:
        ...
        return # ينهي المعالجة هنا لمنع التكرار

    if "إحصائياتي" in text:
        await update.message.reply_text("📊 أهلاً بك! أنت مستخدم مسجل لدينا بنجاح.")
        return

    # --- التحميل المباشر ---
    if "http" in text:
        if status == "not_subbed":
            await update.message.reply_text("❌ اشترك في القنوات أولاً لتفعيل التحميل!")
            return
...
if __name__ == "__main__":
    # رسالة ترحيب في الكونسول (Render Logs)
